...
// Today Coach (coachCard, enhancements.js) is the single authoritative readiness verdict for
// the Training tab. This strip intentionally no longer renders a competing hold/proceed
// decision — it only surfaces the distinct, non-duplicated program-review signal.
function healthStatusStrip(){const program=programStatus(),ar=state.lang==="ar";if(!program.review)return "";return `<section class="health-status review"><div><small>${ar?"حالة البرنامج":"PROGRAM STATUS"}</small><strong>${ar?"موعد مراجعة البرنامج":"Program review due"}</strong></div><span>${ar?`الأسبوع ${program.week}`:`Week ${program.week}`} · v${REP_HEALTH_GUIDE.version}</span></section>`;}
...
function progressionAdvice(id){
  ...
  // Once Insights' Theil-Sen/plateau engine has enough completed sessions for this lift,
  // defer to its verdict instead of recomputing a simpler same-session heuristic that could
  // disagree with it. Below that threshold there's nothing for the engine to analyze yet, so
  // the original current-session-aware heuristic below remains the only signal available.
  const analyzed=window.REP_PERFORMANCE_INSIGHTS?.strength(state)?.exercises?.find(item=>item.exercise===id);
  if(analyzed&&analyzed.sessionCount>=2){
    if(analyzed.plateau)return state.lang==="ar"?"لا تحسّن ملموس منذ أكثر من أسبوعين: ثبّت الوزن، أضف تكراراً نظيفاً، أو خذ أسبوعاً أخف.":"No material improvement in 2+ weeks: hold the load, add a clean rep, or take a lighter week.";
    if(analyzed.recommendation==="reduce")return state.lang==="ar"?"خفّض 5% أو ثبّت الوزن حتى تعود التقنية والتكرارات.":"Reduce about 5% or hold until form and reps recover.";
    if(analyzed.recommendation==="progress")return state.lang==="ar"?`جاهز للتقدم: زد ${jump} كجم في الحصة القادمة.`:`Ready to progress: add ${jump} kg next session.`;
  ...
  if(allTop)return state.lang==="ar"?`${twoWins?"تقدّم مؤكد:":"جاهز للتقدم:"} زد ${jump} كجم في الحصة القادمة.`:`${twoWins?"Progression confirmed:":"Ready to progress:"} add ${jump} kg next session.`;
  return state.lang==="ar"?"ثبّت الوزن؛ ارفع جودة التكرارات أو أكمل 12 تكراراً عند RPE ≤ 7.5.":"Hold the load; improve rep quality or reach 12 reps at RPE ≤ 7.5.";
}
...
// Superseded by enhancements.js's syncStatusText, which always loads after this file and
// reassigns the name — this definition never runs. Kept only as the pre-enhancements fallback
// shape; read the enhancements.js version for actual current behavior.
...
// exportData/importData below are superseded by enhancements.js's encrypted versions
// (exportEncrypted/importSecureBackup) — always loaded after this file, which reassign
// these names before either is ever invoked. Kept only as the pre-encryption fallback shape.
function exportData(){
...
function renderRecovery() {
  ...
  app.innerHTML = `<section class="recovery-head"><p class="eyebrow">Recovery system</p><h1>Adaptation happens here.</h1><p>Use the basics daily. Check in weekly. Pain is information, not a challenge.</p></section>${recoveryDecisionCard()}
    <section class="recovery-grid">
      ...
      <article class="recovery-card"><span class="card-kicker">Daily check-in</span><h2>Log energy, soreness &amp; pain on Vitals</h2><p>The daily check-in — and the readiness score it powers — now lives on the Vitals tab, so there's exactly one place to log it and one place it's scored.</p><button class="module-save" data-goto-vitals>Open Vitals check-in →</button></article>
...
function renderRecoveryArabic(){
  ...
  <article class="recovery-card"><span class="card-kicker">فحص يومي</span><h2>سجّل الطاقة والإجهاد والألم من تبويب الحيوية</h2><p>انتقل الفحص اليومي — ونتيجة الاستعداد التي يغذيها — إلى تبويب الحيوية، فأصبح هناك مكان واحد للتسجيل ومكان واحد للحساب.</p><button class="module-save" data-goto-vitals>افتح فحص الحيوية ←</button></article>
...
// The readiness verdict (hold load / proceed) now lives only in Today Coach on the Vitals tab,
// which uses the weighted health-engine model instead of this screen's simple flag count.
// This card keeps only the program-review signal, which isn't duplicated anywhere else.
function recoveryDecisionCard(){const p=programStatus(),ar=state.lang==="ar";return `<section class="decision-card"><div><small>${ar?"حالة البرنامج":"PROGRAM STATUS"}</small><h2>${ar?`الأسبوع ${p.week}`:`Week ${p.week}`}</h2><p>${p.review?(ar?"موعد مراجعة البرنامج — راجع الأوزان والتمارين المتوقفة.":"Program review due — reassess loads and any stalled lifts."):(ar?"المراجعة القادمة في الأسبوع 8، أو أبكر إذا توقف تمرينان.":"Next review at week 8, or sooner if two lifts stall.")}</p></div>${p.stalled.length>=2?`<div><em>${ar?`${p.stalled.length} تمارين متوقفة`:`${p.stalled.length} lifts stalled`}</em></div>`:""}</section>`;}
// Two elements on this screen link to the Vitals check-in (the sleep summary card and the
// daily check-in pointer), so every match gets bound, not just the first.
function bindRecoveryTools(){document.querySelectorAll("[data-guide-timer]").forEach(b=>b.onclick=()=>startGuideTimer(b.dataset.guideLabel,Number(b.dataset.guideTimer)));document.querySelectorAll("[data-goto-vitals]").forEach(gotoVitals=>{gotoVitals.addEventListener("click",()=>setPrimaryTab("vitals"));gotoVitals.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();setPrimaryTab("vitals");}});});}
...
// Superseded by enhancements.js's foodConnectionCard (adds AI/Notion/Vitals capability
// badges); this definition never runs since enhancements.js always loads after and
// reassigns the name.
...
// weightTrackerCard/waterTrackerCard below are superseded by enhancements.js's versions;
// this file's definitions never run since enhancements.js always loads after and
// reassigns both names.
...
// Superseded by enhancements.js's saveFoodDraft (adds Notion sync-status fields and strips
// photoPreview before persisting); this definition never runs since enhancements.js always
// loads after and reassigns the name.
