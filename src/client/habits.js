(function(){
  const HABITS=[
    ...
    {id:"fasting",icon:"heart",en:"Fasting",ar:"صيام"},
    {id:"quran",icon:"quran",en:"Memorize Quran",ar:"حفظ القرآن"},
    {id:"workout",icon:"workout",en:"Workout",ar:"تمرين"},
    {id:"adhkar",icon:"list",en:"Morning & evening adhkar",ar:"أذكار الصباح والمساء"},
    {id:"read",icon:"book",en:"Read",ar:"قراءة",detailEn:"30 minutes",detailAr:"30 دقيقة"},
  ...
  function completed(date){return HABITS.filter(habit=>checked(date,habit.id));}
  function streak(id){
  ...
  function render(){
    const ar=state.lang==="ar",date=isoDay(),done=completed(date),percent=Math.round(done.length/HABITS.length*100),days=Array.from({length:7},(_,index)=>dateKey(index-6));
  ...
  window.REP_HABITS={definitions:HABITS,bucket,completed,streak,payloadForDate,hasEntries};
