...
(function(){
  ...
  function organizeNutrition(){
    ...
    if(!nav){
      nav=document.createElement("nav");nav.className="module-subnav";nav.dataset.navFor="nutrition";nav.setAttribute("aria-label",ar?"أقسام التغذية":"Nutrition sections");nav.innerHTML=REP_SAFE_DOM.sanitize([["log",ar?"سجّل":"Log"],["today",ar?"اليوم":"Today"],["plan",ar?"الخطة":"Plan"]].map(([id,label])=>`<button data-nutrition-view="${id}" class="${state.nutritionView===id?"is-active":""}">${label}</button>`).join(""));profile.insertAdjacentElement("afterend",nav);
  ...
  renderVitals=function(){state.healthView="vitals";baseVitals();healthNav();document.querySelector(".health-subnav")?.insertAdjacentHTML("afterend",REP_SAFE_DOM.sanitize(`${coachCard(state.lang==="ar")}${importQualityCard(state.lang==="ar")}`));document.querySelector("[data-health-import-check]")?.addEventListener("click",()=>fetchPendingVitals(true));};
  ...
  function organizeTraining(){
    ...
    if(!nav){
      ...
      nav.innerHTML=REP_SAFE_DOM.sanitize([["today",ar?"اليوم":"Today"],["program",ar?"البرنامج":"Program"],["history",ar?"السجل":"History"]].map(([id,label])=>`<button data-training-view="${id}" class="${state.trainingView===id?"is-active":""}">${label}</button>`).join(""));
    ...
    if(toolsTitle&&tools&&!advanced){
      advanced=document.createElement("details");advanced.className="training-advanced";advanced.innerHTML=REP_SAFE_DOM.sanitize(`<summary>${ar?"الأدوات والسلامة":"Tools & safety"}</summary>`);
  ...
  renderHome=function(){
    ...
    anchor?.insertAdjacentHTML("afterend",REP_SAFE_DOM.sanitize(coachCard(state.lang==="ar",true)));
    organizeTraining();
  };
  ...
  function renderSettings(section=state.settingsSection){stopExerciseClock();stopSessionClock();document.body.classList.remove("workout-mode");state.view="settings";state.settingsSection=section;persist();const ar=state.lang==="ar";app.innerHTML=REP_SAFE_DOM.sanitize(`${moduleHeader(ar?"إعدادات":"SETTINGS",ar?"اجعله يناسب يومك.":"Make it fit your life.",ar?"الوحدات والجدول والأهداف والمزامنة والنسخ.":"Units, schedule, targets, sync, and backups.")}${settingsNav(section,ar)}${section==="general"?generalSettings(ar):section==="schedule"?scheduleSettings(ar):section==="targets"?targetsSettings(ar):section==="coach"?coachSettings(ar):section==="sync"?syncCenter.render(state,{ar}):securitySettings(ar)}`);bindSettings(ar);if(section==="sync")bindSyncCenter(ar);document.querySelectorAll("[data-app-tab]").forEach(button=>button.setAttribute("aria-current","false"));}
