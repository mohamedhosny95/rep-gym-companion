...
(function(){
  const APP_SCHEMA=19,features=window.REP_FEATURES,health=window.REP_HEALTH_ENGINE,syncCenter=window.REP_SYNC_CENTER,performance=window.REP_PERFORMANCE_INSIGHTS,adaptive=window.REP_ADAPTIVE_COACH;
  ...
  function applyAdaptiveToday(plan=adaptiveTodayPlan()){
    ...
    adaptive.applyPlan(state,plan,sessions);persist();
  ...
  renderComplete=function(){
    ...
    document.querySelector("[data-accept-progression]")?.addEventListener("click",()=>{adaptive.applyProgression(state,proposals);persist();renderComplete();showToast("Next-session weights are ready.");});
