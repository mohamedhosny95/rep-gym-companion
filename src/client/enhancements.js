...
(function(){
  const APP_SCHEMA=21,features=window.REP_FEATURES,health=window.REP_HEALTH_ENGINE,syncCenter=window.REP_SYNC_CENTER,performance=window.REP_PERFORMANCE_INSIGHTS,adaptive=window.REP_ADAPTIVE_COACH,productSuite=window.REP_PRODUCT_SUITE;
  ...
  const DEFAULT_TARGETS={
    gym:{label:"Gym Day",calories:2250,protein:185,carbs:248,fat:68,fiber:33,water:4200},
    active:{label:"Active Day",calories:2075,protein:175,carbs:0,fat:63,fiber:33,water:3600},
    flex:{label:"Flex Day",calories:2150,protein:175,carbs:0,fat:65,fiber:33,water:3200,calorieCeiling:2480,proteinFloor:160}
  };
  const TARGET_FIELDS=["calories","protein","carbs","fat","fiber","water"];
  const LEGACY_TARGETS={
    gym:[{calories:2380,protein:190,carbs:278,fat:70,fiber:30,water:4200},{calories:2162,protein:176,carbs:248,fat:70,fiber:30,water:3500}],
    active:[{calories:2042,protein:178,carbs:206,fat:66,fiber:30,water:3600},{calories:1990,protein:173,carbs:202,fat:70,fiber:30,water:3200}],
    flex:[{calories:2480,protein:150,carbs:0,fat:70,fiber:30,water:3200},{calories:2480,protein:150,carbs:0,fat:70,fiber:30,water:3000}]
  };
  ...
  state.onboarding=adaptive.normalizeOnboarding(rawSaved.onboarding);
  if(!state.onboarding.completedAt&&Number(rawSaved.version)>0)state.onboarding={...state.onboarding,completedAt:new Date().toISOString(),skipped:true,migrated:true};
  state.activeWorkoutPlan=rawSaved.activeWorkoutPlan&&typeof rawSaved.activeWorkoutPlan==="object"?rawSaved.activeWorkoutPlan:null;
  state.progressionProposals=Array.isArray(rawSaved.progressionProposals)?rawSaved.progressionProposals.slice(0,12):[];
  state.trainingTargets=rawSaved.trainingTargets&&typeof rawSaved.trainingTargets==="object"?rawSaved.trainingTargets:{};
  ...
  state.weekOverrides=rawSaved.weekOverrides&&typeof rawSaved.weekOverrides==="object"?rawSaved.weekOverrides:{};
  state.scheduleAdjustments=Array.isArray(rawSaved.scheduleAdjustments)?rawSaved.scheduleAdjustments.slice(-24):[];
  state.launchEvents=Array.isArray(rawSaved.launchEvents)?rawSaved.launchEvents.slice(-250):[];
  state.customExperiments=productSuite.normalizeExperiments(rawSaved.customExperiments);
  state.experimentCheckins=rawSaved.experimentCheckins&&typeof rawSaved.experimentCheckins==="object"?rawSaved.experimentCheckins:{};
  ...
  function adaptiveTodayPlan(){
    ...
    return adaptive.todayPlan({state,focus:daySchedule().focus,recommendation,date:isoDay()});
  }
  function applyAdaptiveToday(plan=adaptiveTodayPlan()){
    if(state.sessionStartedAt&&state.session===plan.targetSession){showSessionPreview(plan.targetSession);return;}
    adaptive.applyPlan(state,plan,sessions);productSuite.trackEvent(state,"today_plan_applied",{mode:plan.mode,targetSession:plan.targetSession||"none"});persist();
    if(plan.targetSession)showSessionPreview(plan.targetSession);
  ...
  const baseVitals=renderVitals,baseCare=renderHygiene,baseInsights=renderInsights,baseOverview=renderOverview,baseTrainingHome=renderHome,baseComplete=renderComplete;
  ...
  function bindSettings(){
    ...
    document.querySelector("[data-run-onboarding]")?.addEventListener("click",()=>openOnboarding(true));
  ...
  function readOnboardingStep(){
    const dialog=document.querySelector(".onboarding-backdrop");if(!dialog||!onboardingDraft)return;
    const checked=name=>dialog.querySelector(`[name="${name}"]:checked`)?.value;
    onboardingDraft.goal=checked("setup-goal")||onboardingDraft.goal;onboardingDraft.experience=checked("setup-experience")||onboardingDraft.experience;onboardingDraft.healthConnection=checked("setup-health")||onboardingDraft.healthConnection;
    const days=Number(dialog.querySelector("[data-setup-days]")?.value),minutes=Number(dialog.querySelector("[data-setup-minutes]")?.value);if(days)onboardingDraft.daysPerWeek=days;if(minutes)onboardingDraft.preferredSessionMinutes=minutes;
    const equipment=[...dialog.querySelectorAll("[data-setup-equipment]:checked")].map(input=>input.value);if(equipment.length)onboardingDraft.equipment=equipment;
    const weight=checked("setup-weight"),water=checked("setup-water");if(weight)onboardingDraft.weightUnit=weight;if(water)onboardingDraft.waterUnit=water;
    const wake=dialog.querySelector("[data-setup-wake]")?.value,sleep=Number(dialog.querySelector("[data-setup-sleep]")?.value);if(wake)onboardingDraft.wakeTime=wake;if(sleep)onboardingDraft.baseSleepHours=sleep;
  }
  function choice(name,value,label,selected){return `<label class="setup-choice"><input type="radio" name="${name}" value="${value}" ${selected===value?"checked":""}><span>${label}</span></label>`;}
  function onboardingBody(){
    if(onboardingStep===0)return `<fieldset class="setup-fieldset"><legend>${"What should the coach optimize for?"}</legend><div class="setup-choice-grid">${[["strength","Get stronger"],["muscle","Build muscle"],["fat-loss","Lose fat"],["fitness","General fitness"],["recovery","Rebuild safely"]].map(([id,label])=>choice("setup-goal",id,label,onboardingDraft.goal)).join("")}</div></fieldset><fieldset class="setup-fieldset"><legend>${"Training experience"}</legend><div class="setup-choice-grid three">${[["new","New"],["returning","Returning"],["experienced","Experienced"]].map(([id,label])=>choice("setup-experience",id,label,onboardingDraft.experience)).join("")}</div></fieldset>`;
    if(onboardingStep===1)return `<div class="setup-form-grid"><label><span>${"Training days each week"}</span><select data-setup-days>${[2,3,4,5,6].map(value=>`<option value="${value}" ${onboardingDraft.daysPerWeek===value?"selected":""}>${value} days</option>`).join("")}</select></label><label><span>${"Preferred session length"}</span><select data-setup-minutes>${[30,45,60].map(value=>`<option value="${value}" ${onboardingDraft.preferredSessionMinutes===value?"selected":""}>${value} minutes</option>`).join("")}</select></label></div><fieldset class="setup-fieldset"><legend>${"Available equipment"}</legend><div class="setup-choice-grid">${[["machines","Gym machines"],["dumbbells","Dumbbells"],["bodyweight","Bodyweight"],["bands","Bands"],["cardio","Cardio equipment"]].map(([id,label])=>`<label class="setup-choice"><input data-setup-equipment type="checkbox" value="${id}" ${onboardingDraft.equipment.includes(id)?"checked":""}><span>${label}</span></label>`).join("")}</div></fieldset>`;
    return `<div class="setup-form-grid"><fieldset class="setup-fieldset"><legend>${"Weight units"}</legend><div class="setup-choice-grid three">${choice("setup-weight","kg","kg",onboardingDraft.weightUnit)}${choice("setup-weight","lb","lb",onboardingDraft.weightUnit)}</div></fieldset><fieldset class="setup-fieldset"><legend>${"Water units"}</legend><div class="setup-choice-grid three">${choice("setup-water","ml","ml",onboardingDraft.waterUnit)}${choice("setup-water","oz","fl oz",onboardingDraft.waterUnit)}</div></fieldset><label><span>${"Usual wake time"}</span><input data-setup-wake type="time" value="${onboardingDraft.wakeTime}"></label><label><span>${"Starting sleep need"}</span><input data-setup-sleep type="number" min="6" max="10" step="0.1" value="${onboardingDraft.baseSleepHours}"></label></div><fieldset class="setup-fieldset"><legend>${"Connections"}</legend><div class="setup-choice-grid">${choice("setup-health","later","Use the app now · connect later",onboardingDraft.healthConnection)}${choice("setup-health","now","Open connection setup after this",onboardingDraft.healthConnection)}</div></fieldset><p class="setup-note">${"Pairing and Apple Health are optional. Training, recovery check-ins, nutrition, and progression work without them."}</p>`;
  }
  function renderOnboarding(){
    document.querySelector(".onboarding-backdrop")?.remove();const backdrop=document.createElement("div");backdrop.className="onboarding-backdrop";backdrop.innerHTML=REP_SAFE_DOM.sanitize(`<section class="onboarding-dialog" aria-labelledby="onboarding-title"><div class="setup-progress" aria-label="Setup progress"><span class="${onboardingStep>=0?"is-active":""}"></span><span class="${onboardingStep>=1?"is-active":""}"></span><span class="${onboardingStep>=2?"is-active":""}"></span></div><small>${`STEP ${onboardingStep+1} OF 3`}</small><h1 id="onboarding-title">${["Build your plan","Fit it to your week","Finish the essentials"][onboardingStep]}</h1><p>${["Choose the outcome and starting level. You can change everything later.","Set the time and equipment the plan can actually use.","Set units and sleep timing. Device connections stay optional."][onboardingStep]}</p><div class="onboarding-body">${onboardingBody()}</div><div class="onboarding-actions">${onboardingStep?`<button data-onboarding-back type="button">${"Back"}</button>`:`<button data-onboarding-skip type="button">${"Skip for now"}</button>`}<button class="settings-primary" data-onboarding-next type="button">${onboardingStep===2?("Finish setup"):("Continue")}</button></div></section>`);document.body.appendChild(backdrop);
    backdrop.setAttribute("aria-label","Guided setup");
    backdrop.querySelector("[data-onboarding-back]")?.addEventListener("click",()=>{readOnboardingStep();onboardingStep--;renderOnboarding();});
    backdrop.querySelector("[data-onboarding-skip]")?.addEventListener("click",()=>{state.onboarding={...state.onboarding,completedAt:new Date().toISOString(),skipped:true};persist();backdrop.remove();});
  ...
  const DIALOG_SELECTORS=".timed-mode,.exit-confirm,.install-help,.rep-modal-backdrop,.plate-calc-backdrop,.onboarding-backdrop";
