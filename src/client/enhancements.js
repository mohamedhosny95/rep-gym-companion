...
(function(){
  ...
  const DEFAULT_SCHEDULE={
    ...
    Wednesday:{morning:true,focus:"cardio"},Thursday:{morning:true,focus:"gym"},Friday:{morning:false,focus:"rest"},Saturday:{morning:false,focus:"activespa"}
  };
  ...
  function normalizedPreferences(value={}){
    ...
    for(const day of DAY_NAMES){const incoming=value.schedule?.[day];if(incoming){schedule[day].morning=Boolean(incoming.morning);if(["gym","cardio","recovery","spa","rest","activespa"].includes(incoming.focus))schedule[day].focus=incoming.focus;}}
  ...
  function focusLabel(focus,ar=state.lang==="ar"){const labels={gym:["Gym","جيم"],cardio:["Cardio","كارديو"],recovery:["Active recovery","استشفاء نشط"],spa:["Spa recovery","استشفاء السبا"],rest:["Rest","راحة"],activespa:["Active recovery + Spa","استشفاء نشط + سبا"]};return labels[focus]?.[ar?1:0]||focus;}
  ...
  renderNutrition=function(){
    ...
    document.querySelectorAll(".food-entry").forEach((card,index)=>{
      ...
      if(entry&&actions&&!actions.querySelector("[data-save-favorite]")){
        ...
        button.textContent=isFav?(ar?"★ مفضلة":"★ Favorite"):(ar?"☆ مفضلة":"☆ Favorite");
    ...
    if(reset)reset.onclick=()=>{const previous=Number(state.water[isoDay()])||0;if(!previous)return;setFoodWater(0);showUndo(state.lang==="ar"?"تمت إعادة مياه اليوم.":"Today's water was reset.",()=>setFoodWater(previous));};
    organizeNutrition();
  };
  ...
  function organizeTraining(){
    ...
    let nav=document.querySelector(".module-subnav[aria-label*='Training'],.module-subnav[aria-label*='التدريب']");
    if(!nav){
      nav=document.createElement("nav");nav.className="module-subnav";nav.setAttribute("aria-label",ar?"أقسام التدريب":"Training sections");
      nav.innerHTML=[["today",ar?"اليوم":"Today"],["program",ar?"البرنامج":"Program"],["history",ar?"السجل":"History"]].map(([id,label])=>`<button data-training-view="${id}" class="${state.trainingView===id?"is-active":""}">${label}</button>`).join("");
    ...
    if(toolsTitle&&tools&&!advanced){
      advanced=document.createElement("details");advanced.className="training-advanced";advanced.innerHTML=`<summary>${ar?"الأدوات والسلامة":"Tools & safety"}</summary>`;
    ...
    if(state.trainingView==="today"){
      const focus=daySchedule().focus,id=focus==="gym"?"gym":focus==="cardio"?"cardio":"bad";
  ...
  renderHome=function(){
    ...
    anchor?.insertAdjacentHTML("afterend",coachCard(state.lang==="ar",true));
    organizeTraining();
  };
  ...
  function pwaInstallCard(ar){
    const isStandalone=window.navigator.standalone===true||window.matchMedia('(display-mode: standalone)').matches;
    if(isStandalone)return `<section class="settings-card" style="border-color:rgba(201,255,61,.3);background:linear-gradient(145deg,rgba(201,255,61,.06),var(--panel));"><small style="color:var(--acid);font-weight:900;">${ar?"تطبيق مثبت":"INSTALLED APP"}</small><h2>${ar?"Health OS يعمل كتطبيق مستقل":"Health OS is running as a Standalone PWA"}</h2><p style="margin:0;color:var(--muted);font-size:11px;">✓ ${ar?"شاشة كاملة بدون شريط متصفح وبدون انقطاع.":"Full screen with fast offline caching and no browser bar."}</p></section>`;
    return `<section class="settings-card pwa-install-card" style="border-color:rgba(125,201,255,.3);background:linear-gradient(145deg,rgba(125,201,255,.07),var(--panel));"><small style="color:var(--blue);font-weight:900;">${ar?"تثبيت على الآيفون":"INSTALL ON IPHONE"}</small><h2>${ar?"أضف Health OS للشاشة الرئيسية":"Add to Home Screen for Native Experience"}</h2><div style="display:grid;gap:8px;margin:10px 0 12px;font-size:12px;color:var(--text);"><div style="display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:10px;background:rgba(255,255,255,.04);"><b>1</b><span>${ar?"اضغط زر المشاركة (⎋) في أسفل متصفح Safari":"Tap the <b>Share (⎋)</b> icon at the bottom of Safari"}</span></div><div style="display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:10px;background:rgba(255,255,255,.04);"><b>2</b><span>${ar?"اختر <b>إضافة إلى الشاشة الرئيسية (+)</b>":"Scroll down and tap <b>Add to Home Screen (+)</b>"}</span></div></div><button class="settings-primary" data-install-settings style="background:var(--blue);color:#03202e;">${ar?"تثبيت / تعليمات":"Install / Add to Home Screen"}</button></section>`;
  }
  function circadianRemindersCard(ar){
    return `<section class="settings-card push-card"><small style="color:var(--acid);font-weight:900;">${ar?"الإشعارات والتذكير اليومي":"CIRCADIAN REMINDERS & NOTIFICATIONS"}</small><h2>${ar?"تنبيهات الاستشفاء والنوم الذكية":"Smart Readiness & Wind-Down Alarms"}</h2>
      <div style="display:grid;gap:10px;margin:12px 0;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-radius:12px;background:var(--panel-2);">
          <div><strong>🌅 ${ar?"تنبيه الاستشفاء الصباحي":"Morning Readiness Score"}</strong><small style="display:block;color:var(--muted);font-size:10px;">${ar?"07:30 صباحاً · ملخص جاهزية الجسم وتوصية اليوم":"07:30 AM · Readiness score and training guidance"}</small></div>
          <span style="color:var(--acid);font-weight:900;">07:30</span>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-radius:12px;background:var(--panel-2);">
          <div><strong>🌙 ${ar?"تذكير موعد النوم والاسترخاء":"Evening Bedtime Wind-Down"}</strong><small style="display:block;color:var(--muted);font-size:10px;">${ar?"22:30 مساءً · بدء الاسترخاء قبل موعد النوم بـ 45 دقيقة":"10:30 PM · Wind-down reminder 45m before sleep"}</small></div>
          <span style="color:#7dc9ff;font-weight:900;">22:30</span>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-radius:12px;background:var(--panel-2);">
          <div><strong>📖 ${ar?"تذكير سورة الكهف (الجمعة)":"Surat Al-Kahf Friday Reminder"}</strong><small style="display:block;color:var(--muted);font-size:10px;">${ar?"09:00 صباح كل جمعة":"09:00 AM every Friday"}</small></div>
      ...
      <div style="display:flex;gap:8px;margin-top:10px;">
        <button type="button" data-local-notify-test class="quiet-setting" style="flex:1;">🔔 ${ar?"اختبار إشعار تجريبي الآن":"Send Test Notification Now"}</button>
      </div>
    </section>`;
  }
  function generalSettings(ar){
    ...
    return `${pwaInstallCard(ar)}<section class="settings-card"><small>${ar?"تفضيلاتك":"YOUR PREFERENCES"}</small><h2>${ar?"اجعل التطبيق مألوفاً":"Make the app feel familiar"}</h2>
      ...
      <p>${ar?"تُحفظ القيم داخلياً بالكيلوجرام والمل، لذلك لا تتغير سجلاتك عند تبديل العرض.":"Stored values stay in kilograms and milliliters, so switching display units never changes your history."}</p></section>
      ${circadianRemindersCard(ar)}
      <section class="settings-card data-migration-card"><small>${ar?"استيراد وترحيل البيانات السابقة":"DATA MIGRATION & BACKFILL"}</small><h2>${ar?"استيراد من Strong أو Hevy أو Apple Health":"Import from Strong, Hevy, or Apple Health"}</h2><p>${ar?"ارفع ملف CSV أو XML لاستيراد سجلات التمارين والتغذية والوزن السابقة وحساب القوة فوراً.":"Drop your CSV or XML export to backfill historical workouts, body weights, and meals."}</p><label class="settings-primary" style="display:inline-block;cursor:pointer;text-align:center;padding:10px 16px;margin-top:8px;">📥 ${ar?"اختر ملف الاستيراد (CSV/XML)":"Choose File (CSV/XML)"}<input type="file" accept=".csv,.xml,.json" data-import-file style="display:none;"></label><span class="import-status-msg" data-import-status style="display:block;margin-top:8px;font-size:12px;color:var(--acid);"></span></section>`;
  }
  function scheduleSettings(ar){const arabicDays={Sunday:"الأحد",Monday:"الاثنين",Tuesday:"الثلاثاء",Wednesday:"الأربعاء",Thursday:"الخميس",Friday:"الجمعة",Saturday:"السبت"};return `<section class="settings-card"><small>${ar?"أسبوعك":"YOUR WEEK"}</small><h2>${ar?"اختر نوع كل يوم":"Choose each day's focus"}</h2><div class="schedule-editor">${DAY_NAMES.map(day=>{const plan=daySchedule(day),label=ar?arabicDays[day]:day.slice(0,3);return `<div class="schedule-row"><strong>${label}</strong><select data-schedule-focus="${day}">${["gym","cardio","recovery","spa","rest","activespa"].map(focus=>`<option value="${focus}" ${plan.focus===focus?"selected":""}>${focusLabel(focus,ar)}</option>`).join("")}</select><label><input data-schedule-morning="${day}" type="checkbox" ${plan.morning?"checked":""}> ${ar?"تنشيط":"Activation"}</label></div>`;}).join("")}</div><button class="quiet-setting" data-reset-schedule>${ar?"إعادة الجدول الافتراضي":"Reset default schedule"}</button></section>`;}
  ...
  function renderSettings(section=state.settingsSection){stopExerciseClock();stopSessionClock();document.body.classList.remove("workout-mode");state.view="settings";state.settingsSection=section;persist();const ar=state.lang==="ar";app.innerHTML=`${moduleHeader(ar?"إعدادات":"SETTINGS",ar?"اجعله يناسب يومك.":"Make it fit your life.",ar?"الوحدات والجدول والأهداف والمزامنة والنسخ.":"Units, schedule, targets, sync, and backups.")}${settingsNav(section,ar)}${section==="general"?generalSettings(ar):section==="schedule"?scheduleSettings(ar):section==="targets"?targetsSettings(ar):section==="coach"?coachSettings(ar):section==="sync"?syncCenter.render(state,{ar}):securitySettings(ar)}`;bindSettings(ar);if(section==="sync")bindSyncCenter(ar);document.querySelectorAll("[data-app-tab]").forEach(button=>button.setAttribute("aria-current","false"));}
  ...
  function bindSettings(ar){
    document.querySelectorAll("[data-settings-tab]").forEach(button=>button.onclick=()=>renderSettings(button.dataset.settingsTab));
    document.querySelectorAll("[data-language]").forEach(button=>button.onclick=()=>{if(state.lang!==button.dataset.language)document.querySelector("#langButton")?.click();renderSettings("general");});
    ...
    document.querySelector("[data-local-notify-test]")?.addEventListener("click",async()=>{
      if("Notification" in window){
        if(Notification.permission!=="granted"){
          ...
          if(p!=="granted"){showToast(ar?"يرجى السماح بالإشعارات من إعدادات المتصفح.":"Please grant notification permission in browser settings.");return;}
        }
        try{
          const reg=await navigator.serviceWorker?.ready;
          if(reg?.showNotification) await reg.showNotification(ar?"⚡ Health OS: استشفاء اليوم 88%":"⚡ Health OS: Today's Readiness 88%",{body:ar?"جاهزية عالية للدفع اليوم. خطة اليوم: تمرين الصدر والظهر.":"High readiness today. Today's plan: Chest & Back.",icon:"./icon-192.png",badge:"./icon-192.png"});
          else new Notification(ar?"⚡ Health OS: استشفاء اليوم 88%":"⚡ Health OS: Today's Readiness 88%",{body:ar?"جاهزية عالية للدفع اليوم.":"High readiness today."});
          showToast(ar?"تم إرسال إشعار تجريبي بنجاح!":"Test notification sent successfully!");
        }catch(err){
          try{new Notification(ar?"⚡ Health OS: تجربة التنبيه":"⚡ Health OS: Test Notification",{body:ar?"الإشعارات تعمل بنجاح!":"Notifications are working perfectly!"});showToast(ar?"تم إرسال إشعار تجريبي بنجاح!":"Test notification sent successfully!");}catch(e){showToast(String(err.message||err));}
        }
      }else{showToast(ar?"الإشعارات غير مدعومة في هذا المتصفح.":"Notifications are not supported in this browser.");}
    });
