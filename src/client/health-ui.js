...
(function(){
  ...
  const labelsAr={sleep:["النوم","س",1],hrv:["تقلب القلب","مل ث",0],rhr:["نبض الراحة","bpm",0],resp:["التنفس","/د",1],vo2:["أقصى استهلاك أكسجين","",1]};
  ...
  function trendCard(){
    ...
    return `<section class="long-term-card"><div><small>${ar()?"خط أساس شخصي":"PERSONAL BASELINE"}</small><h2>${ar()?"اتجاهات 7 · 28 · 90 يوماً":"7 · 28 · 90 day trends"}</h2><p>${ar()?"نقارن جسمك بنفسه، لا بمتوسط السكان. الاتجاهات أهم من قراءة يوم واحد.":"Your body is compared with its own history, not a population grade. Trends matter more than one reading."}</p></div>
      <div class="trend-table" role="table" tabindex="0" aria-label="${ar()?"اتجاهات الصحة طويلة المدى":"Long-term health trends"}"><div class="trend-row trend-head" role="row"><span role="columnheader">${ar()?"المقياس":"Metric"}</span><span role="columnheader">${ar()?"الآن":"Now"}</span><span role="columnheader">7d</span><span role="columnheader">28d</span><span role="columnheader">90d</span></div>
      ${report.metrics.map(metric=>{const [label,unit,digits]=(ar()?labelsAr:labels)[metric.name];return `<div class="trend-row" role="row"><strong role="rowheader">${label}</strong><span role="cell">${num(metric.current,digits)} ${unit}</span><span role="cell">${num(metric.average7,digits)}</span><span role="cell">${num(metric.average28,digits)}</span><span role="cell">${num(metric.average90,digits)}</span></div>`;}).join("")}</div>
      <div class="measurement-summary"><span><b>${num(report.weight.current,1)} kg</b>${ar()?"آخر وزن":"Latest weight"}</span><span><b>${num(report.waistCm,1)} cm</b>${ar()?"آخر محيط خصر":"Latest waist"}</span></div>
      <form data-body-measurement><label>${ar()?"الوزن (كجم)":"Weight (kg)"}<input name="weight" type="number" min="30" max="300" step=".1"></label><label>${ar()?"الخصر (سم)":"Waist (cm)"}<input name="waist" type="number" min="40" max="250" step=".1"></label><label>${ar()?"الانقباضي":"Systolic"}<input name="systolic" type="number" min="70" max="250"></label><label>${ar()?"الانبساطي":"Diastolic"}<input name="diastolic" type="number" min="40" max="150"></label><button type="submit">${ar()?"حفظ القياسات":"Save measurements"}</button><button type="button" class="quiet" data-health-report>${ar()?"تصدير التقرير":"Export report"}</button><output aria-live="polite"></output></form>
