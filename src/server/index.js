...
async function storeVitalsImport(env,vitals){
  ...
  const record={...merged,date:vitals.date,imported_at:importedAt,import_runs:[...new Set([...priorRuns,importedAt])].sort().slice(-24)};
...
async function monitorSystemHealth(env){
  ...
  if(previous?.checkedAt&&Date.now()-Date.parse(previous.checkedAt)<6*60*1000)return previous;
