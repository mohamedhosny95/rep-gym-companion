...
function renderHome() {
  ...
  app.innerHTML = `
    ...
    <section class="session-grid training-tools" aria-label="${state.lang==="ar"?"أدوات التمرين":"Training tools"}">
      ...
      <button class="session-card" data-log-activity style="--card-accent:#ffd36a"><span><small>${state.lang==="ar"?"بادل · كرة قدم · المزيد":"PADEL · FOOTBALL · MORE"}</small><h2>${state.lang==="ar"?"تسجيل نشاط":"Log an activity"}</h2></span><span class="session-icon">+</span><p>${state.lang==="ar"?"رياضات غير منظمة: المدة والجهد وتقدير للسعرات المحروقة.":"Unstructured sports — duration, effort, and an estimated calorie burn."}</p><small>${state.lang==="ar"?"سجّل الآن ←":"Log now →"}</small></button>
...
function logActivity(type,customName,minutes,rpe,notes){
  ...
  const calories=estimateCalories(type,duration),rpeValue=Math.max(1,Math.min(10,Math.round(Number(rpe))||6)),note=String(notes||"").trim().slice(0,200);
  const record={id:Date.now(),date:new Date().toISOString(),session:"activity",activityType:type,activityLabel:label,duration,calories,sets:0,loads:{},entries:[{entry:`${label} · ${mins} min`,exercise:label,set:1,weight:"",reps:"",rpe:rpeValue,note,duration,rest:"",progression:"",personalBest:false}],cardio:null};
...
function showLogActivity(){
  ...
  box.innerHTML=`<strong>${ar?"تسجيل نشاط":"Log an activity"}</strong>
    <p class="activity-hint">${ar?"للرياضات غير المنظمة مثل البادل وكرة القدم — مدة وجهد بدلاً من قائمة تمارين موجهة.":"For unstructured sports like padel and football — duration and effort, not a guided exercise list."}</p>
    ...
    <div class="activity-form-grid"><label>${ar?"المدة (دقيقة)":"Duration (min)"}<input type="number" data-activity-minutes min="1" max="300" step="5" value="60" inputmode="numeric"></label><label>RPE<input type="number" data-activity-rpe min="1" max="10" step="1" value="6" inputmode="numeric"></label></div>
  ...
  document.body.appendChild(box);
  let selected=ACTIVITY_TYPES[0][0];
  ...
  box.querySelector("[data-save-activity]").onclick=()=>{
    const minutes=box.querySelector("[data-activity-minutes]").value,rpe=box.querySelector("[data-activity-rpe]").value,notes=box.querySelector("[data-activity-notes]").value,custom=box.querySelector("[data-activity-custom]").value;
    if(logActivity(selected,custom,minutes,rpe,notes)){
