...
const state = {
  ...
  foodEntries:saved.foodEntries||[], savedMeals:saved.savedMeals||[], water:saved.water||{}, foodDraft:null, foodMealType:saved.foodMealType||"", foodLogMethod:"Ingredients", foodBusy:false,
...
function persist() {
  localStorage.setItem(storageKey, JSON.stringify({ version:5, guideVersion:REP_HEALTH_GUIDE.version, activeTab:state.activeTab, session: state.session, index: state.index, completed: state.completed, muted: state.muted, checkin: saved.checkin || {}, lang:state.lang, speed:state.speed, paused:state.paused, muscles:state.muscles, viewMode:state.viewMode, logs:state.logs, swaps:state.swaps, history:state.history, sessionStartedAt:state.sessionStartedAt, reviews:state.reviews, fieldTest:state.fieldTest, voice:state.voice, syncQueue:state.syncQueue, recoveryCheckins:state.recoveryCheckins, daily:state.daily, cardioDraft:state.cardioDraft, programStart:state.programStart, foodEntries:state.foodEntries, savedMeals:state.savedMeals, water:state.water, foodMealType:state.foodMealType }));
}
...
function setPrimaryTab(tab){state.activeTab=tab;persist();updatePrimaryTabs();if(tab==="food")renderNutrition();else if(tab==="care")renderCareHub();else renderHome();}
function renderCareHub(){stopExerciseClock();stopSessionClock();document.body.classList.remove("workout-mode");state.activeTab="care";state.view="care";persist();updatePrimaryTabs();const ar=state.lang==="ar",gate=recoveryGate(),bucket=dailyBucket("hygiene"),done=Object.values(bucket.checked||{}).filter(Boolean).length;app.innerHTML=`${moduleHeader(ar?"العناية":"CARE",ar?"الاستشفاء والعناية، في مكان واحد.":"Recover. Reset. Repeat.",ar?"افتح مراجعة الاستشفاء أو روتين العناية اليومي دون الرجوع إلى الصفحة الرئيسية.":"Recovery decisions and daily care stay one tap away.")}${recoveryDecisionCard()}<section class="care-grid"><button class="care-card" data-care-recovery style="--care-accent:#d9b3ff"><div><small>${ar?"قرار اليوم":"READINESS"}</small><h2>${ar?"الاستشفاء":"Recovery"}</h2></div><b>≈</b><p>${gate.stale?(ar?"سجّل مراجعة سريعة لتفعيل بوابة التقدم.":"Log a quick check-in to activate progression gating."):(ar?`${gate.flags} علامات خطر في آخر مراجعة.`:`${gate.flags} red flags in your latest check-in.`)}</p></button><button class="care-card" data-care-hygiene style="--care-accent:#73e6d1"><div><small>${ar?"روتين اليوم":"DAILY ROUTINE"}</small><h2>${ar?"العناية اليومية":"Daily care"}</h2></div><b>✦</b><p>${ar?`${done} عناصر مكتملة اليوم. افتح الصباح والمساء والشعر وما بعد التمرين.`:`${done} items checked today. Open morning, evening, hair, and post-workout care.`}</p></button><button class="care-card" data-care-review style="--care-accent:#ef6f55"><div><small>${ar?"السلامة والجودة":"SAFETY & QUALITY"}</small><h2>${ar?"المراجعة والاختبار":"Review & field test"}</h2></div><b>✓</b><p>${ar?"اعتماد مختص، فحص الحركة، واختبار الاستخدام داخل الجيم.":"Professional sign-off, movement checks, and real-gym usability testing."}</p></button></section>`;document.querySelector("[data-care-recovery]").onclick=renderRecovery;document.querySelector("[data-care-hygiene]").onclick=renderHygiene;document.querySelector("[data-care-review]").onclick=renderReview;}
...
function renderReview(){
  state.view="review";state.activeTab="care";persist();updatePrimaryTabs();document.body.classList.remove("workout-mode");const ar=state.lang==="ar",r=state.reviews,complete=reviewExercises.filter(x=>r[x]?.signed).length;
...
function renderRecovery() {
  state.view="recovery";state.activeTab="care";persist();updatePrimaryTabs();
...
function logFoodWater(amount){state.water[isoDay()]=(Number(state.water[isoDay()])||0)+amount;queueNutritionSummary();persist();renderNutrition();}
...
updatePrimaryTabs();state.activeTab==="food"?renderNutrition():state.activeTab==="care"?renderCareHub():renderHome();
