...
function hasMeaningfulData(){return state.history.length>0||state.foodEntries.length>0||state.recoveryCheckins.length>0||state.bodyWeights.length>0||state.mealTemplates.length>0||Object.keys(state.logs||{}).length>0||Object.keys(state.daily?.hygiene||{}).length>0;}
...
function mealTemplatesSection(ar){
  if(!state.mealTemplates.length)return "";
  return `<div class="food-section-head"><h2>${ar?"قوالب الوجبات":"Meal templates"}</h2><span>${ar?"اضغط للمراجعة ثم التأكيد":"Tap to review, then confirm"}</span></div><section class="meal-templates">${state.mealTemplates.map(t=>`<div class="meal-template"><button data-use-template="${esc(t.id)}"><strong>${esc(t.food_name)}</strong><span>${Math.round(Number(t.calories)||0)} kcal · P ${Math.round(Number(t.protein_g)||0)}g</span></button><button class="template-remove" data-delete-template="${esc(t.id)}" aria-label="${ar?"حذف القالب":"Delete template"}">×</button></div>`).join("")}</section>`;
}
