...
async function syncPending(){
  ...
  for(const item of [...state.syncQueue]){
    try{const legacy=item.workout&&!item.kind,body=(legacy||item.kind==="workout")?{workout:item.workout}:{kind:item.kind,payload:item.payload},id=item.id||`workout-${item.workout?.id}`;const response=await repAuth.fetch("/api/notion-sync",{method:"POST",headers:{"content-type":"application/json","x-rep-idempotency-key":id},body:JSON.stringify(body)}),data=await response.json().catch(()=>({}));const receiptMatches=data.verified===true&&(item.kind==="workout"||Boolean(data.notionPageId))&&(item.kind!=="food"||(data.kind==="food"&&data.entryId===item.payload?.id));if(!response.ok||!data.ok||!receiptMatches){const error=Error(data.error||"Notion did not return a verified save receipt.");error.auth=response.status===401;throw error;}if(item.kind==="food"){const entry=state.foodEntries.find(food=>food.id===item.payload.id);if(entry){entry.notionSync="synced";entry.notionUrl=data.notionUrl||"";entry.notionPageId=data.notionPageId;entry.notionSyncedAt=new Date().toISOString();}}state.syncQueue=state.syncQueue.filter(q=>(q.id||`workout-${q.workout?.id}`)!==id);state.lastSyncedAt=new Date().toISOString();persist();}
    catch(error){item.attempts=(item.attempts||0)+1;item.error=String(error.message||error).slice(0,180);if(error.auth){repAuth.clear();state.syncState="auth";state.pairMessage=state.lang==="ar"?"انتهى اقتران الجهاز. أعد الاتصال.":"Device pairing expired. Reconnect.";}else state.syncState="failed";persist();if(state.view==="nutrition")renderNutrition();else updateSyncPanel();return;}
  }
