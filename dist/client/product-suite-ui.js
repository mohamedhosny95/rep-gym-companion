/* UI adapters for the local-first product suite. */
(function(){
  ...
  async function handlePrivateLink(){
    ...
    try{const result=await suite.readPrivateWeeklyLink(match[1]),html=reportCard.generateReportHtml(result.payload,{privateView:true});document.open();document.write(html);document.close();reportCard.bindActions(document,window);return true;}catch(error){document.body.textContent=String(error.message||error);return true;}
  }
  function weeklyCard(){
  ...
  function experimentsCard(){
    ...
    return `<section class="product-card experiment-studio" data-product-experiments><div class="product-card-head"><div><small>PERSONAL OUTCOME LAB</small><h2>Run your own 90-day experiments</h2></div><span>${rows.length} active</span></div><p>Track one behavior, then compare next-day readiness. Results unlock after at least 5 “yes” and 5 “no” days.</p><form data-experiment-add><div class="experiment-presets">${suite.PRESETS.map(item=>`<button type="button" data-experiment-preset="${item.id}">${esc(item.label)}</button>`).join("")}</div><label><span>Custom habit or behavior</span><input data-experiment-label maxlength="80" placeholder="e.g. Magnesium before bed"><button class="settings-primary">Add experiment</button></label></form><div class="experiment-list">${rows.map(row=>`<article><div><strong>${esc(row.label)}</strong><small>${row.ready?`${row.effect>0?"+":""}${row.effect} readiness-point association`:`${row.yesDays}/5 yes · ${row.noDays}/5 no`}</small></div><div class="binary-check" role="group" aria-label="${esc(row.label)} today"><button data-experiment-check="${esc(row.id)}" data-value="true" class="${checks[row.id]===true?"is-active":""}">Yes</button><button data-experiment-check="${esc(row.id)}" data-value="false" class="${checks[row.id]===false?"is-active":""}">No</button><button data-experiment-delete="${esc(row.id)}" aria-label="Delete ${esc(row.label)}">×</button></div>${row.ready?`<p><b>${row.yesAverage}</b> with · <b>${row.noAverage}</b> without. ${esc(row.boundary)}</p>`:""}</article>`).join("")||'<p class="product-empty">Add a behavior above. Everything stays on this device and in your encrypted backup.</p>'}</div><small class="association-note">Association, not causation. Change one behavior at a time and keep training, sleep, and logging as consistent as practical.</small></section>`;
  }
  ...
  function bindWeekly(card){
    card.querySelector("[data-weekly-pdf]").onclick=()=>reportCard.openPrintableReport(state);
    card.querySelector("[data-weekly-link]").onclick=async()=>{try{const link=await suite.createPrivateWeeklyLink(suite.weeklySummary(state,undefined,window.REP_PERFORMANCE_INSIGHTS),location.href);await navigator.clipboard.writeText(link);suite.trackEvent(state,"weekly_report_shared",{method:"private_link"});save();toast("Encrypted weekly link copied. It expires in 7 days.");}catch(error){toast(String(error.message||error));}};
  }
  function bindExperiments(card){
    const add=label=>{label=String(label||"").trim();if(!label)return;state.customExperiments=suite.normalizeExperiments([...(state.customExperiments||[]),{id:`experiment-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,label,preset:"custom",createdAt:new Date().toISOString()}]);save();mount(true);};
    card.querySelectorAll("[data-experiment-preset]").forEach(button=>button.onclick=()=>add(suite.PRESETS.find(row=>row.id===button.dataset.experimentPreset)?.label));
    card.querySelector("[data-experiment-add]").onsubmit=event=>{event.preventDefault();add(card.querySelector("[data-experiment-label]").value);};
    card.querySelectorAll("[data-experiment-check]").forEach(button=>button.onclick=()=>{const today=suite.dateKey();state.experimentCheckins[today]={...(state.experimentCheckins[today]||{}),[button.dataset.experimentCheck]:button.dataset.value==="true"};save();mount(true);});
    card.querySelectorAll("[data-experiment-delete]").forEach(button=>button.onclick=()=>{state.customExperiments=state.customExperiments.filter(row=>row.id!==button.dataset.experimentDelete);for(const values of Object.values(state.experimentCheckins))delete values[button.dataset.experimentDelete];save();mount(true);});
  }
  async function enableSmartReminders(reminders){
    if(!reminders.some(row=>row.enabled))throw Error("Select at least one reminder first.");if(!("serviceWorker" in navigator)||!("PushManager" in window))throw Error("Push reminders are not supported in this browser.");
    const keyResponse=await fetch("/api/push/public-key"),keyData=await keyResponse.json();if(!keyResponse.ok||!keyData.key)throw Error("Push is not configured on the server.");if(await Notification.requestPermission()!=="granted")throw Error("Notification permission was not granted.");
  ...
  async function loadPhotos(card){
    photoUrls.forEach(URL.revokeObjectURL);photoUrls=[];const items=await features.listProgressPhotos({includeBytes:true}),grid=card.querySelector("[data-photo-grid]");
    grid.innerHTML=REP_SAFE_DOM.sanitize(items.length?items.map((item,index)=>{const metric=measurementFor(item.date);return `<article><label><input type="checkbox" data-photo-select="${esc(item.id)}" ${index<2?"checked":""}><span>Compare</span></label><img data-photo-image="${esc(item.id)}" alt="${esc(item.label)}"><strong>${esc(item.label)}</strong><small>${item.date}${metric.weight?` · ${metric.weight} kg`:""}${metric.waist?` · waist ${metric.waist} cm`:""}</small><button data-photo-delete="${esc(item.id)}">Delete</button></article>`;}).join(""):'<p class="product-empty">No photos yet. Add an optional encrypted check-in above.</p>');
    for(const item of items){const url=URL.createObjectURL(new Blob([item.bytes],{type:item.mime}));photoUrls.push(url);grid.querySelector(`[data-photo-image="${CSS.escape(item.id)}"]`).src=url;}
    const compare=()=>{const selected=[...grid.querySelectorAll("[data-photo-select]:checked")].slice(0,2),panel=card.querySelector("[data-photo-compare]");panel.hidden=selected.length!==2;if(selected.length===2){panel.querySelector("[data-compare-a]").src=grid.querySelector(`[data-photo-image="${CSS.escape(selected[0].dataset.photoSelect)}"]`).src;panel.querySelector("[data-compare-b]").src=grid.querySelector(`[data-photo-image="${CSS.escape(selected[1].dataset.photoSelect)}"]`).src;}};grid.querySelectorAll("[data-photo-select]").forEach(input=>input.onchange=compare);grid.querySelectorAll("[data-photo-delete]").forEach(button=>button.onclick=async()=>{await features.deleteProgressPhoto(button.dataset.photoDelete);await loadPhotos(card);});compare();
  }
  function bindPhotos(card){card.querySelector("[data-photo-add]").onsubmit=async event=>{event.preventDefault();const file=card.querySelector("[data-photo-file]").files[0];if(!file)return toast("Choose a photo first.");try{const bytes=await processImage(file);await features.saveProgressPhoto({bytes,date:card.querySelector("[data-photo-date]").value,label:card.querySelector("[data-photo-label]").value||"Progress photo",mime:"image/jpeg"});suite.trackEvent(state,"progress_photo_saved",{});save();await loadPhotos(card);toast("Photo encrypted on this device.");}catch(error){toast(String(error.message||error));}};loadPhotos(card).catch(()=>{const grid=card.querySelector("[data-photo-grid]");grid.textContent="The encrypted photo vault is unavailable in this browser.";});}
  ...
  handlePrivateLink().then(opened=>{if(opened)return;const app=document.querySelector("#app");new MutationObserver(()=>queueMicrotask(()=>mount())).observe(app,{childList:true,subtree:false});mount();});
})();
