...
function hasMeaningfulData(){return state.history.length>0||state.foodEntries.length>0||state.recoveryCheckins.length>0||state.bodyWeights.length>0||Object.keys(state.logs||{}).length>0||Object.keys(state.daily?.hygiene||{}).length>0;}
function notionProtected(){return Boolean(localStorage.getItem(syncKeyStorage))&&state.syncQueue.length===0&&!["failed","auth"].includes(state.syncState);}
...
function insightsPanel(){
  const items=buildInsights();if(!items.length)return "";
  const ar=state.lang==="ar";
  return `<section class="insights-card"><div class="insights-head"><small>${ar?"ملخص عام":"WHAT THE DATA SAYS"}</small></div>${items.map(i=>`<p class="insight insight-${i.tone}">${esc(i.text)}</p>`).join("")}</section>`;
}
...
function bindDaily(kind,render){document.querySelectorAll("[data-daily-key]").forEach(el=>el.onchange=()=>{const b=dailyBucket(kind);b.checked[el.dataset.dailyKey]=el.checked;persist();render();});const notes=document.querySelector("[data-daily-notes]");if(notes)notes.oninput=e=>{dailyBucket(kind).notes=e.target.value;persist();};}
...
function weekKey(d=new Date()){const x=new Date(d);x.setHours(0,0,0,0);x.setDate(x.getDate()-x.getDay());return x.toISOString().slice(0,10);}
function currentWeekWeight(){return state.bodyWeights.find(w=>w.week===weekKey())||null;}
function saveBodyWeight(kg){const value=Math.round(Number(kg)*10)/10;if(!Number.isFinite(value)||value<30||value>300)return false;const week=weekKey();state.bodyWeights=state.bodyWeights.filter(w=>w.week!==week);state.bodyWeights.unshift({week,date:isoDay(),kg:value});state.bodyWeights=state.bodyWeights.slice(0,104);persist();return true;}
function deleteBodyWeight(week){state.bodyWeights=state.bodyWeights.filter(w=>w.week!==week);persist();}
